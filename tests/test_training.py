...
def test_training_metrics_are_strong_enough_for_demo() -> None:
    ...
    assert artifacts.metrics["selected_model_role"] in {"champion", "challenger", "torch"}
    assert artifacts.metrics["selected_model_framework"] in {"sklearn", "torch"}
    ...
    assert artifacts.metrics["torch"]["model_version"] == "model-v1-torch"
    ...
    assert manifest["active_model_framework"] in {"sklearn", "torch"}
    assert manifest["active_model_role"] in {"champion", "challenger", "torch"}
    assert manifest["available_models"]["model-v1-torch"]["framework"] == "torch"
    assert manifest["available_models"]["model-v1-torch"]["model_file"] == str(TORCH_MODEL_FILE)
